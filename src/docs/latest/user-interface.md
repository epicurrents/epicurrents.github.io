...
- **Dataset display**: The dataset display is on the left side of the signal viewport and includes both the dataset selector dropdown and the navigator below it. The dataset display can be fast-toggled from the menu icon (three vertical bars) on the left edge of the resource controls bar (below) and resources in the open dataset can be switched with the arrows next to the menu icon.
